# -*- coding: utf-8 -*-
# @Author : liang
# @File : cif2dataset_bulk_moduli.py
...
# from utils.packag import build_undirected_edgedata, nearest_neighbor_edges_submit
import torch
import itertools
import gzip, pickle
...
def convert(args):
    ...
    ############################megnet######################################
    dataset1 = pd.read_pickle('/home/zl/DGM/dataset/finetuned/raw/bulk_megnet_train.pkl')
    dataset2 = pd.read_pickle('/home/zl/DGM/dataset/finetuned/raw/bulk_megnet_val.pkl')
    dataset3 = pd.read_pickle('/home/zl/DGM/dataset/finetuned/raw/bulk_megnet_test.pkl')
    ...
    for atoms in tqdm(dataset):
        ...
        bulk_list.append(atoms['bulk modulus'].numpy().max())
    ...
    # df = pd.DataFrame(columns=['id', 'atom_txt', 'graph_input', 'bulk modulus', 'shear modulus'])
    #
    # df['id'] = id_list
    # df['atom_txt'] = atoms_list
    # df['graph_input'] = graphs_list
    # df['bulk modulus'] = bulk_list
    # df['shear modulus'] = shear_list

    torch.save({'id': id_list,
                'atom_txt': atoms_list,
                'graph_input': graphs_list,
                'bulk modulus': bulk_list,
                # 'shear modulus': shear_list,
                },
               args.save_path + 'bulk_moduli_processed.pt')
